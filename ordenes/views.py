...
# Sirve para verificar que el usuario esta logado y redirige al template de login
    ...
    if request.method == "POST":
        ...
        if producto.stock > 0:
            ...
            if id in request.session['articulos']:
                request.session['articulos'][id]['cuantos'] = request.session['articulos'][id]['cuantos']+int(cantidad)
            else:
                request.session['articulos'][id] = {'precio': float(producto.precio),'cuantos':int(cantidad)}
...
def comprar(request):
    ...
    for articulo in articulos:
        ...
        precio = articulos[articulo]['precio']
        cantidad = articulos[articulo]['cuantos']
        detail = DetalleOrden(producto=prod,orden=orden,cantidad=cantidad,precio=precio)
        detail.save()
